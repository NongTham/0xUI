...
-- เลือก Theme ตรงนี้ (Themes.Default หรือ Themes.Halloween)
local CurrentTheme = Themes.Halloween 

--// Pre-defined Sounds
local Sounds = {
	Button = Instance.new("Sound", SoundService),
	Enter = Instance.new("Sound", SoundService),
...
local function SafeLoadIcons()
	pcall(function()
		Library.Icons = loadstring(game:HttpGet('https://raw.githubusercontent.com/HJSIWN/SomtankLib/refs/heads/main/Icon.lua'))()
	end)
end
...
local function MakeDraggable(dragHandle, moveTarget)
	...
	dragHandle.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			if dragHandle.Name == "DragIconButton" then Library.State.DragIconOldPosition = moveTarget.Position end
...
local function AnimateButton(button, playSound)
	...
	if not Library.State.OldPosition[name] and not button:GetAttribute("IsDragIcon") then Library.State.OldPosition[name] = button.Position end
	if playSound and not Library.State.NoSound then Sounds.Button:Play() end
...
local function UpdateCanvasSize(scrollingFrame)
	...
	if layout then scrollingFrame.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 20) end
end
...
function SomtankUI:CreateWindow(Settings)
	...
	-- Main Frame (ImageLabel for Background)
	...
	local function CreateTopButton(text, pos, callback)
		...
		btn.Activated:Connect(function() AnimateButton(btn, true); if callback then callback() end end)
		return btn
	end

	CreateTopButton("X", UDim2.new(0.91, 0, 0.046, 0), function()
		Tween(MainFrame, {BackgroundTransparency = 1}, 0.3).Completed:Wait()
		ScreenGui:Destroy()
	end)
	CreateTopButton("-", UDim2.new(0.80, 0, 0.046, 0), function() isWindowOpen = false; MainFrame.Visible = false end)
	...
	Resizer.Text = ""
	local resizeDrag, resizeStartPos, resizeStartScale = false, nil, nil
	Resizer.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			resizeDrag = true; resizeStartPos = input.Position; resizeStartScale = UIScale.Scale
		end
	end)
	UserInputService.InputChanged:Connect(function(input)
	...
	UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then resizeDrag = false end end)
	...
	function Tabs:Tab(options)
		...
		if options.Icon then SetIcon(options.Icon, TabIconBG, UDim2.fromScale(0.8, 0.8), UDim2.fromScale(0.1, 0.1), CurrentTheme.Accent) end
		...
		local function ActivateTab()
			for _, t in pairs(TabScroll:GetChildren()) do
				if t:IsA("Frame") and t ~= TabFrame then
					Tween(t.Frame, {Size = UDim2.new(0.3, 0, 1, 0)}, 0.3)
					Tween(t.TextLabel, {TextTransparency = 1}, 0.2)
				end
			end
			...
			Page.Visible = true
			if not Library.State.NoSound then Sounds.Enter:Play() end
		end
		...
		function Elements:MiniTab(options)
			...
			SectionFrame.Size = UDim2.new(0, 191, 0, 210)
			...
			SectionLabel.Text = options.Title or "Section"
			...
			SectionLayout.Padding = UDim.new(0.02, 0); SectionLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
			...
			function ItemFuncs:Click(options)
				...
				label.Size = UDim2.new(0.7, 0, 1, 0); label.Position = UDim2.fromScale(0.05, 0)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Button"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)
				local clickIcon = Instance.new("ImageButton", frame)
				clickIcon.Size = UDim2.fromOffset(25, 25); clickIcon.Position = UDim2.new(1, -30, 0.5, -12.5)
				clickIcon.BackgroundTransparency = 1; clickIcon.Image = "rbxassetid://75478984455074"
				clickIcon.ImageColor3 = CurrentTheme.Accent
				local trigger = Instance.new("TextButton", frame)
				trigger.Size = UDim2.fromScale(1, 1); trigger.BackgroundTransparency = 1; trigger.Text = ""
				trigger.Activated:Connect(function()
					AnimateButton(clickIcon, true)
					clickIcon.ImageColor3 = Color3.new(1,1,1)
			...
			function ItemFuncs:Toggle(options)
				local frame = CreateItemBase(40)
				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(0.6, 0, 1, 0); label.Position = UDim2.fromScale(0.05, 0)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Toggle"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)
				local toggleBg = Instance.new("Frame", frame)
				toggleBg.Size = UDim2.fromOffset(40, 20); toggleBg.Position = UDim2.new(1, -50, 0.5, -10)
				...
				toggleDot.Size = UDim2.fromOffset(16, 16); toggleDot.Position = UDim2.new(0, 2, 0.5, -8)
				...
				local function UpdateToggle(animated)
					if state then
						Tween(toggleDot, {Position = UDim2.new(1, -18, 0.5, -8), BackgroundColor3 = Color3.new(1,1,1)}, animated and 0.2 or 0)
						Tween(toggleBg, {BackgroundColor3 = CurrentTheme.Accent}, animated and 0.2 or 0)
					else
						Tween(toggleDot, {Position = UDim2.new(0, 2, 0.5, -8), BackgroundColor3 = CurrentTheme.TextColor2}, animated and 0.2 or 0)
						Tween(toggleBg, {BackgroundColor3 = CurrentTheme.BackgroundColor1}, animated and 0.2 or 0)
					end
				end
				UpdateToggle(false)
				trigger.Activated:Connect(function()
					state = not state; UpdateToggle(true)
			...
			function ItemFuncs:Slider(options)
				...
				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(1, -10, 0, 20); label.Position = UDim2.fromScale(0.05, 0.1)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Slider"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)
				local valLabel = Instance.new("TextLabel", frame)
				valLabel.Size = UDim2.new(0, 50, 0, 20); valLabel.Position = UDim2.new(1, -55, 0.1, 0)
				valLabel.BackgroundTransparency = 1; valLabel.Text = tostring(default)
				valLabel.Font = Enum.Font.FredokaOne; valLabel.TextColor3 = Color3.new(1,1,1); valLabel.TextSize = 14
				local sliderBar = Instance.new("Frame", frame)
				sliderBar.Size = UDim2.new(0.9, 0, 0, 6); sliderBar.Position = UDim2.new(0.05, 0, 0.7, 0)
				sliderBar.BackgroundColor3 = CurrentTheme.BackgroundColor1
				AddDecoration(sliderBar, "Corner", 1, 0)
				local fill = Instance.new("Frame", sliderBar)
				fill.Size = UDim2.new(0, 0, 1, 0); fill.BackgroundColor3 = CurrentTheme.Accent
				AddDecoration(fill, "Corner", 1, 0)
				local trigger = Instance.new("TextButton", frame)
				trigger.Size = UDim2.new(0.9, 0, 0.5, 0); trigger.Position = UDim2.new(0.05, 0, 0.5, 0)
				...
				trigger.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						dragging = true; UpdateDrag(input)
					end
				end)
				UserInputService.InputChanged:Connect(function(input)
					if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			-- SliderEnum
			function ItemFuncs:SliderEnum(options)
				local items = options.Options or {"A", "B"}
				local defaultIdx = options.Default or 1
				local frame = CreateItemBase(60)
				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(1, -10, 0, 20); label.Position = UDim2.fromScale(0.05, 0.1)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Mode"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)

				local valLabel = Instance.new("TextLabel", frame)
				valLabel.Size = UDim2.new(0, 100, 0, 20); valLabel.Position = UDim2.new(1, -105, 0.1, 0)
				valLabel.BackgroundTransparency = 1; valLabel.Text = items[defaultIdx] or "..."
				valLabel.Font = Enum.Font.FredokaOne; valLabel.TextColor3 = Color3.new(1,1,1); valLabel.TextSize = 14
				...
				local function SetIndex(idx)
					idx = math.clamp(idx, 1, #items)
					local percent = (idx - 1) / (#items - 1)
					if #items == 1 then percent = 1 end
					fill.Size = UDim2.new(percent, 0, 1, 0)
					valLabel.Text = items[idx]
					if options.Callback then options.Callback(items[idx], idx) end
				end
				SetIndex(defaultIdx)
				local function UpdateDrag(input)
					local percent = math.clamp((input.Position.X - sliderBar.AbsolutePosition.X) / sliderBar.AbsoluteSize.X, 0, 1)
					local idx = math.floor(percent * (#items - 1) + 0.5) + 1
					SetIndex(idx)
				end
				trigger.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			-- Input
			function ItemFuncs:Input(options)
				local frame = CreateItemBase(40)
				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(0.4, 0, 1, 0); label.Position = UDim2.fromScale(0.05, 0)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Input"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)

				local box = Instance.new("TextBox", frame)
				box.Size = UDim2.new(0.5, 0, 0.8, 0); box.Position = UDim2.new(0.45, 0, 0.1, 0)
				box.BackgroundColor3 = CurrentTheme.BackgroundColor1; box.TextColor3 = Color3.new(1,1,1)
				box.PlaceholderText = options.Placeholder or "..."
				box.Text = ""; box.Font = Enum.Font.FredokaOne
				AddDecoration(box, "Corner", 0, 4)
			...
			function ItemFuncs:Dropdown(options)
				...
				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(0.4, 0, 1, 0); label.Position = UDim2.fromScale(0.05, 0)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Dropdown"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)
				local displayFrame = Instance.new("Frame", frame)
				displayFrame.Size = UDim2.new(0.5, 0, 0.7, 0); displayFrame.Position = UDim2.new(0.45, 0, 0.15, 0)
				...
				currentText.Size = UDim2.new(0.8, 0, 1, 0); currentText.Position = UDim2.fromScale(0.1, 0)
				currentText.BackgroundTransparency = 1; currentText.Text = defaultItem
				currentText.TextColor3 = Color3.new(1,1,1); currentText.Font = Enum.Font.FredokaOne
				currentText.TextScaled = true
				local arrow = Instance.new("ImageLabel", displayFrame)
				arrow.Size = UDim2.fromOffset(15, 15); arrow.Position = UDim2.new(1, -20, 0.5, -7.5)
				...
				listFrame.Name = "DropdownList"; listFrame.Size = UDim2.fromOffset(displayFrame.AbsoluteSize.X, 0)
				listFrame.BackgroundColor3 = CurrentTheme.BackgroundColor1; listFrame.BorderSizePixel = 0
				listFrame.Visible = false; listFrame.ScrollBarThickness = 2; listFrame.ZIndex = 100
				...
				local function Close()
					...
					Tween(listFrame, {Size = UDim2.fromOffset(displayFrame.AbsoluteSize.X, 0)}, 0.2).Completed:Connect(function()
						listFrame.Visible = false; Library.State.ActiveDropdown = nil
					end)
					Tween(arrow, {Rotation = 0}, 0.2)
				end
				local function Open()
					if Library.State.ActiveDropdown and Library.State.ActiveDropdown ~= Close then Library.State.ActiveDropdown() end
					isOpen = true; listFrame.Visible = true
				...
				local function Populate()
					...
					for _, item in pairs(items) do
						...
						btn.Size = UDim2.new(1, 0, 0, 25); btn.BackgroundTransparency = 1
						btn.Text = item; btn.TextColor3 = Color3.fromRGB(200, 200, 200)
						btn.Font = Enum.Font.FredokaOne; btn.TextSize = 14; btn.ZIndex = 101
						...
						btn.Activated:Connect(function()
							currentText.Text = item; if options.Callback then options.Callback(item) end; Close()
						end)
					end
					listFrame.CanvasSize = UDim2.new(0, 0, 0, #items * 27)
				end
				Populate()
				trigger.Activated:Connect(function() if isOpen then Close() else Open() end end)
				RunService.RenderStepped:Connect(function()
					if isOpen and displayFrame.Visible then
						listFrame.Position = UDim2.fromOffset(displayFrame.AbsolutePosition.X, displayFrame.AbsolutePosition.Y + displayFrame.AbsoluteSize.Y + 5)
					elseif isOpen and not displayFrame.Visible then Close() end
				end)
			end

			-- Frame3DShow (Fixed Archivable)
			function ItemFuncs:Frame3DShow(options)
				local frame = CreateItemBase(100) -- Taller base
				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(1, -10, 0, 20); label.Position = UDim2.fromScale(0.05, 0.05)
				label.BackgroundTransparency = 1; label.Text = options.Title or "3D View"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)

				local vpFrame = Instance.new("ViewportFrame", frame)
				vpFrame.Size = UDim2.new(0.9, 0, 0.7, 0); vpFrame.Position = UDim2.new(0.05, 0, 0.25, 0)
				vpFrame.BackgroundColor3 = CurrentTheme.BackgroundColor1
				vpFrame.BackgroundTransparency = 0.5
				AddDecoration(vpFrame, "Corner", 0, 6)

				local camera = Instance.new("Camera"); vpFrame.CurrentCamera = camera
				local worldModel = Instance.new("WorldModel", vpFrame)

				local Model = nil
				local Spin = options.Setting and options.Setting.Spin or true
				...
				local function SetModel(target)
					...
					task.spawn(function()
						while Model and Model.Parent do
							if Spin then rot = rot + 0.02 end
							camera.CFrame = CFrame.Angles(0, rot, 0) * CFrame.new(0, 0, dist)
				...
				if options.ItemShow then SetModel(options.ItemShow) end

				local Funcs = {}
				function Funcs:ShowThisItem(item) SetModel(item) end
				return Funcs
			end

			-- LineGraph
			function ItemFuncs:LineGraph(options)
				local frame = CreateItemBase(200) -- Taller base
				frame.BackgroundColor3 = Color3.fromRGB(48, 35, 75)

				local label = Instance.new("TextLabel", frame)
				label.Size = UDim2.new(1, -10, 0, 20); label.Position = UDim2.fromScale(0.05, 0.05)
				label.BackgroundTransparency = 1; label.Text = options.Title or "Graph"
				label.Font = Enum.Font.FredokaOne; label.TextColor3 = CurrentTheme.TextColor2
				label.TextSize = 16; label.TextXAlignment = Enum.TextXAlignment.Left
				AddDecoration(label, "Stroke", CurrentTheme.UIStroke1, 2)

				local graphArea = Instance.new("Frame", frame)
				graphArea.Size = UDim2.new(0.9, 0, 0.75, 0); graphArea.Position = UDim2.new(0.05, 0, 0.2, 0)
				graphArea.BackgroundTransparency = 1

				local function DrawGraph(values)
					graphArea:ClearAllChildren()
					if not values or #values < 2 then return end

					local maxVal = 100 -- Default max
					for _, v in pairs(values) do if v > maxVal then maxVal = v end end

					local width = graphArea.AbsoluteSize.X
					local height = graphArea.AbsoluteSize.Y
					local points = {}

					for i, val in ipairs(values) do
						local x = ((i - 1) / (#values - 1)) * width
						local y = height - ((val / maxVal) * height)

						local p = Instance.new("Frame", graphArea)
						p.Size = UDim2.fromOffset(6, 6); p.Position = UDim2.fromOffset(x - 3, y - 3)
						p.BackgroundColor3 = Color3.fromRGB(255, 185, 7)
						AddDecoration(p, "Corner", 1, 0)
						table.insert(points, {x=x, y=y})
					end

					for i = 1, #points - 1 do
						local p1, p2 = points[i], points[i+1]
						local line = Instance.new("Frame", graphArea)
				...
				if options.Values then DrawGraph(options.Values) end

				local Funcs = {}
				function Funcs:UpdateGraph(newOpts)
					if newOpts.Values then DrawGraph(newOpts.Values) end
				end
				return Funcs
			end
